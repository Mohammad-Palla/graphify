...
def _extract_python_rationale(
    path: Path, result: dict, parsed: "tuple | None" = None
) -> None:
    ...
    ``parsed`` is the ``(tree, source)`` the structural pass already produced.
    Without it this function re-parses the file from scratch -- a SECOND
    tree-sitter parse of every Python file in the corpus, which measured ~14% of
    Python extraction (0.55s of 3.85s over 800 django files). Parse is C on both
    sides and is the one cost no walker can reduce, so the fix is to not pay it
    twice. Same duplicate-parse the collect_js fusion removed for JS/TS.

    None is still accepted and still re-parses: callers that have no tree (the
    native-kernel path returns before parsing) must keep working.
    """
    try:
